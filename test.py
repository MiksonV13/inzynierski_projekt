...
class App(tk.Tk):
    def __init__(self):
        ...
        style.configure('TButton', font=('Segoe UI', 10, 'bold'), padding=10, background="#1976D2", foreground="#ffffff")
        ...
        nb.add(self.t_cost, text="💰 Różnica i koszt")
        nb.add(self.t_hist, text="📚 Historia")
        nb.add(self.t_edit, text="✏️ Edycja")
        nb.add(self.t_chart, text="📊 Wykresy")
    ...
    def build_read(self):
        ...
        f2 = ttk.Frame(frm);
        f2.pack(fill="x", padx=pad, pady=8)
    ...
    def build_cost(self):
        ...
        self.e_sel = ttk.Combobox(top_combo_frame, textvariable=self.e_sel_var, state="readonly", 
                                  width=40, font=('Segoe UI', 10))
        self.w_sel = ttk.Combobox(top_combo_frame, textvariable=self.w_sel_var, state="readonly", 
                                  width=40, font=('Segoe UI', 10))
        self.g_sel = ttk.Combobox(top_combo_frame, textvariable=self.g_sel_var, state="readonly", 
                                  width=40, font=('Segoe UI', 10))
        ...
        for label in [self.cost_label_elec, self.cost_label_water, self.cost_label_gas]:
            label.pack(anchor="w", pady=(8, 0))
        
        chart_frame = ttk.LabelFrame(right_panel, text="Wykres kosztów")
        chart_frame.pack(fill="both", expand=True, padx=8, pady=(8, 0))
        
        self.chart_e_frame = ttk.Frame(chart_frame)
        self.chart_w_frame = ttk.Frame(chart_frame)
        self.chart_g_frame = ttk.Frame(chart_frame)
    ...
    def _switch_cost_type(self, kind):
        for frame in [self.chart_e_frame, self.chart_w_frame, self.chart_g_frame]:
            frame.pack_forget()
        for combo in [self.e_sel, self.w_sel, self.g_sel]:
            combo.pack_forget()
        for label in [self.cost_label_elec, self.cost_label_water, self.cost_label_gas]:
            label.pack_forget()
        ...
        if kind == "prąd":
            self.e_sel.pack(fill="x")
            self.cost_label_elec.pack(anchor="w", pady=(8, 0))
            self.chart_e_frame.pack(fill="both", expand=True)
            self.calc_cost_elec()
        elif kind == "woda":
            self.w_sel.pack(fill="x")
            self.cost_label_water.pack(anchor="w", pady=(8, 0))
            self.chart_w_frame.pack(fill="both", expand=True)
            self.calc_cost_water()
        else:
            self.g_sel.pack(fill="x")
            self.cost_label_gas.pack(anchor="w", pady=(8, 0))
    ...
    def refresh_e_combo(self):
        ...
        if opts:
            self.e_sel.current(len(opts) - 1)
            self.calc_cost_elec()
        self.cost_label_elec.config(text="")
    ...
    def refresh_w_combo(self):
        ...
        if opts:
            self.w_sel.current(len(opts) - 1)
            self.calc_cost_water()
        self.cost_label_water.config(text="")
    ...
    def refresh_g_combo(self):
        ...
        if opts:
            self.g_sel.current(len(opts) - 1)
            self.calc_cost_gas()
        self.cost_label_gas.config(text="")
    ...
    def calc_cost_elec(self):
        ...
        if total_readings < 2:
            self.cost_label_elec.config(text="Potrzebne są co\nnajmniej 2 odczyty")
            self._clear_cost_chart("elec")
            return
        ...
        if len(hist) == 1:
            ...
            before_prev = float(hist[-3]["value"]) if len(hist) >= 3 else (float(initial) if initial is not None else prev)
            diff_curr = curr - prev
            diff_prev = prev - before_prev
        
        if diff_curr < 0 or diff_prev < 0:
            self.cost_label_elec.config(text="Błąd:\nSpadek licznika")
            self._clear_cost_chart("elec")
            return
        
        cost_curr = diff_curr * tariff if tariff else 0
        cost_prev = diff_prev * tariff if tariff else 0
        txt = f"Bieżący: {cost_curr:.2f} zł\nPoprzedni: {cost_prev:.2f} zł"
        self.cost_label_elec.config(text=txt)
        self._draw_cost_chart("elec", cost_curr, cost_prev)
    ...
    def calc_cost_water(self):
        ...
        if total_readings < 2:
            self.cost_label_water.config(text="Potrzebne są co\nnajmniej 2 odczyty")
            self._clear_cost_chart("water")
            return
        ...
        if diff_curr < 0 or diff_prev < 0:
            self.cost_label_water.config(text="Błąd:\nSpadek licznika")
            self._clear_cost_chart("water")
            return
        ...
        cost_prev = diff_prev * tariff if tariff else 0
        txt = f"Bieżący: {cost_curr:.2f} zł\nPoprzedni: {cost_prev:.2f} zł"
        self.cost_label_water.config(text=txt)
        self._draw_cost_chart("water", cost_curr, cost_prev)
    ...
    def calc_cost_gas(self):
        ...
        if total_readings < 2:
            self.cost_label_gas.config(text="Potrzebne są co\nnajmniej 2 odczyty")
            self._clear_cost_chart("gas")
            return
        ...
        if diff_curr < 0 or diff_prev < 0:
            self.cost_label_gas.config(text="Błąd:\nSpadek licznika")
            self._clear_cost_chart("gas")
            return
        ...
        cost_prev = diff_prev * tariff if tariff else 0
        txt = f"Bieżący: {cost_curr:.2f} zł\nPoprzedni: {cost_prev:.2f} zł"
        self.cost_label_gas.config(text=txt)
        self._draw_cost_chart("gas", cost_curr, cost_prev)
    ...
    def _draw_cost_chart(self, kind, current_cost, previous_cost):
        if kind == "elec":
            frame = self.chart_e_frame
        elif kind == "water":
            frame = self.chart_w_frame
        ...
        bars = ax.bar(labels, costs, color=colors, edgecolor=['#1565C0', '#D32F2F'], linewidth=2)
        
        title_size = max(figsize_h * 1.5, 9)
        label_size = max(figsize_h, 8)
        
        ax.set_ylabel('Koszt (zł)', fontsize=label_size, fontweight='bold')
        ax.set_title('Porównanie kosztów', fontsize=title_size, fontweight='bold', pad=20)
        ...
        value_size = max(figsize_h * 0.8, 6)
        
        for bar, cost in zip(bars, costs):
            height = bar.get_height()
            ax.text(bar.get_x() + bar.get_width()/2., max_cost_val * 1.05,
                   f'{cost:.2f} zł',
                   ha='center', va='bottom', fontsize=value_size, fontweight='bold', color='#333333')
    ...
    def refresh_h_combo(self):
        ...
        opts = [f'{m["name"]} (ID: {m["id"][:8]})' for m in meters]
    ...
    def load_history_table(self):
        ...
        initial = m.get("initial_reading")
        if initial is not None:
            self.tbl_hist.insert("", "end", values=("Początkowy", f'{float(initial):.3f}'), tags=('initial',))
        
        for i, r in enumerate(history_readings(m["id"]), 1):
            self.tbl_hist.insert("", "end", values=(f"Odczyt #{i}", f'{float(r["value"]):.3f}'), tags=('reading', r["id"]))
    ...
    def build_edit(self):
        ...
        ttk.Label(frm, text="Liczniki do edycji:", font=('Segoe UI', 11, 'bold')).pack(anchor="w", padx=pad, pady=(pad, 8))
    ...
    def delete_selected_meter(self):
        ...
        if messagebox.askyesno("Potwierdzenie", f"Usunąć licznik '{meter['name']}'?\nWszystkie jego odczyty zostaną usunięte."):
    ...
    def build_chart(self):
        ...
        ttk.Label(frm, text="Wybierz licznik:", font=('Segoe UI', 10, 'bold')).pack(anchor="w", padx=pad, pady=(pad, 8))
        self.c_sel_var = tk.StringVar()
        self.c_sel = ttk.Combobox(frm, textvariable=self.c_sel_var, state="readonly", width=48, font=('Segoe UI', 10))
        self.c_sel.pack(padx=pad, pady=(0, 12))
        
        btn_frame = ttk.Frame(frm)
        btn_frame.pack(padx=pad, pady=(0, 12), fill="x")
        ttk.Button(btn_frame, text="📊 Porównanie", command=lambda: self._switch_chart_type("comparison")).pack(side="left", padx=4)
        ttk.Button(btn_frame, text="📈 Historia", command=lambda: self._switch_chart_type("history")).pack(side="left", padx=4)
        ttk.Button(btn_frame, text="🔄 Odśwież", command=self.update_chart).pack(side="left", padx=4)
        
        self.chart_frame = ttk.Frame(frm)
        self.chart_frame.pack(fill="both", expand=True, padx=pad, pady=(0, pad))
        
        self._chart_type = "comparison"
        self.refresh_chart_combo()
    
    def _switch_chart_type(self, chart_type):
        self._chart_type = chart_type
        self.update_chart()
    
    def refresh_chart_combo(self):
        meters = load_meters()
        self._meters_cache_c = meters
        opts = [f'{m["name"]} ({m["kind"]})' for m in meters]
        self.c_sel["values"] = opts
        if opts:
            self.c_sel.current(len(opts) - 1)
            self.update_chart()
        self.c_sel.bind("<<ComboboxSelected>>", lambda e: self.update_chart())
    
    def update_chart(self):
        idx = self.c_sel.current()
        if idx < 0 or not getattr(self, "_meters_cache_c", None):
            return
        m = self._meters_cache_c[idx]
        ...
        if total_readings < 2:
            ttk.Label(self.chart_frame, text="Potrzebne są co najmniej 2 odczyty do wyświetlenia wykresu.", 
                     font=('Segoe UI', 10)).pack(padx=16, pady=16)
            return
        
        self.after(100, lambda: self._draw_chart_matplotlib(m, hist))
    
    def _draw_chart_matplotlib(self, m, hist):
        ...
        if self._chart_type == "comparison":
            ...
            if len(hist) >= 1:
                ...
                bars = ax.bar(bar_labels, bar_values, color=colors_comparison, edgecolor=['#1565C0', '#D32F2F'], linewidth=2)
                ax.set_ylabel('Wartość [' + self._get_unit(m["kind"]) + ']', fontsize=11, fontweight='bold')
                ax.set_title('Porównanie: Bieżący vs Ostatni', fontsize=13, fontweight='bold', pad=20)
                ax.grid(axis='y', alpha=0.3)
                
                max_val = max(bar_values)
                for i, (bar, v) in enumerate(zip(bars, bar_values)):
                    ax.text(bar.get_x() + bar.get_width()/2., max_val * 1.05, f'{v:.3f}', ha='center', va='bottom', fontsize=11, fontweight='bold')
        else:
